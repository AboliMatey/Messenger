	...
	if !ok {
		...
		if operation[0] == "changeName" {
			...
			if reflect.DeepEqual(session_info, (dao.Validation_info{})) {
				...
				if validators.Validate_for_delcontact(t) {
					grp, _ := json.Marshal(dao.Update_add_block_list(t, "DEL"))
					return (string(grp)), 200
				} else {
					return ("Delete Contact request denied !!"), 400
				}
			}
