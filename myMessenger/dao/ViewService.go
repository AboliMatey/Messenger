...
type View_info struct {
	...
	Msg_ids []bson.ObjectId		  	`json:"msg_ids"`
}

func Add_single_view_id(sid string,oid string,vid bson.ObjectId) {
  ...
  for _,v := range other_info.Block_list{
	  if v == bson.ObjectIdHex(sid){
...
func Update_view(t View_update_struct) View_info {
	...
	c.Update(bson.M{"_id": bson.ObjectIdHex(t.View_id)}, bson.M{"$set": bson.M{"msg_ids":result.Msg_ids}})
	c.Find(bson.M{"_id": bson.ObjectIdHex(t.View_id)}).One(&result)
	return result
}
