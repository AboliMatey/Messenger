...
func Get_other_user_info(t Other_user_get_struct) Other_user_info {
	...
	result := User_info{}
	c.Find(bson.M{"cont_id": bson.ObjectIdHex(t.Other_cont_id)}).One(&result)
...
func Change_user_status(t User_change_status) User_info {
	...
	c := session.DB("myMessenger").C("users")
	result := User_info{}
	c.Update(bson.M{"_id": bson.ObjectIdHex(t.User_id)}, bson.M{"$set": bson.M{"user_status": t.New_User_status}})
	c.Find(bson.M{"_id": bson.ObjectIdHex(t.User_id)}).One(&result)
	return result
}
...
func Change_user_passwd(t User_change_passwd) User_info {
	...
	c := session.DB("myMessenger").C("users")
	result := User_info{}
	c.Update(bson.M{"_id": bson.ObjectIdHex(t.User_id)}, bson.M{"$set": bson.M{"passwd": t.New_passwd}})
	c.Find(bson.M{"_id": bson.ObjectIdHex(t.User_id)}).One(&result)
	return result
}
...
func Update_add_block_list(t Add_Block_contact, opt string) User_contect {
	...
	if opt == "ADD" {
		...
		for i,v := range result.Cont_list{
			if v == bson.ObjectIdHex(t.Member_cont_id) {
				result.Cont_list = append(result.Cont_list[:i],result.Cont_list[i+1:]...)
			}
		}
		c.Update(bson.M{"_id": bson.ObjectIdHex(t.Self_cont_id)}, bson.M{"$set": bson.M{"cont_list": result.Cont_list}})
	} else if opt == "BLOCK" {
		result.Block_list = append(result.Block_list, bson.ObjectIdHex(t.Member_cont_id))
		for i,v := range result.Sviews{
			if v.Cont_id == bson.ObjectIdHex(t.Member_cont_id){
				result.Sviews = append(result.Sviews[:i],result.Sviews[i+1:]...)
			}
		}
		c.Update(bson.M{"_id": bson.ObjectIdHex(t.Self_cont_id)}, bson.M{"$set": bson.M{"block_list": result.Block_list ,"sviews" : result.Sviews}})
	}
	c.Find(bson.M{"_id": bson.ObjectIdHex(t.Self_cont_id)}).One(&result)
	return result
}
