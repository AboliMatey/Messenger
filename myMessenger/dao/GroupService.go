...
func Create_group(g Group_post_struct) Group_info {
  ...
  for _,v := range g.Gmembers {
    ...
    cont_info.Group_list = append(cont_info.Group_list,group_id)
    c.Update(bson.M{"_id": bson.ObjectIdHex(v)}, bson.M{"$set": bson.M{"gviews":cont_info.Gviews,"group_list":cont_info.Group_list}})
  }
